...
import { emptyPagingVideosData, getAllVideosByOption } from '../store/slices/paginationSlice';
...
function SearchResult() {
    ...
    useEffect(() => {
        ...
        dispatch(getAllVideosByOption({ search: query, page: 1, limit: 6 }))
                .unwrap()
                .then(() => {fetchedPageRef.current.add(1)})
    ...
    const handleScroll = () => {
        ...
        if( scrolledValue + 5 > scrollHeight) {
            ...
            if(currentPagingInfo.hasNextPage && 
                !fetchedPageRef.current.has(currentPagingInfo.nextPage)
            ) {
                fetchedPageRef.current.add(currentPagingInfo.nextPage);
                dispatch(getAllVideosByOption({ search: query, page: `${currentPagingInfo.nextPage}`, limit: 6 }))
            }
        }
    }
